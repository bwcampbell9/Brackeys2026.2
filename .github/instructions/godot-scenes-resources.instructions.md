...
- Use scenes for reusable node composition and Resources for typed, serializable, Inspector-editable data. Use lightweight GDScript classes or `RefCounted` objects for transient rules that need no Godot lifecycle or serialization.
...
- Use `snake_case` for GDScript files, methods, signals, and non-exported members. Preserve existing PascalCase exported property names because `.tscn` and `.tres` serialization is name-based. Keep node names and exact path casing stable for export portability.
